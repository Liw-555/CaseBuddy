...
# ─── 强制 UTF-8 输出，解决 Windows GBK 控制台编码问题 ───
# 当 stdout 被重定向到文件（nohup/日志文件）时，Windows 默认用 GBK 编码
# 含有中文的字符串（如 LLM 返回内容）会触发 UnicodeEncodeError
os.environ['PYTHONUTF8'] = '1'

def _ensure_utf8(stream):
    """安全地将 stream 包装为 UTF-8，如果已经是则跳过"""
    try:
...
# 覆盖 builtins.print，确保所有 print() 自动 UTF-8 且永不崩溃
...
try:
    ...
    WeComBot = None
    print("[Gateway] 企业微信Bot模块未找到，企业微信功能不可用。请检查 bots/wecom_bot.py")
...
class GatewayState:
    def __init__(self):
        ...
        self.config = {
            ...
            'feishu': {'enabled': False, 'logged_in': False, 'appId': '', 'secret': '', 'allowedUsers': []},
            'wecom': {'enabled': False, 'logged_in': False, 'corpId': '', 'agentId': '', 'secret': '', 'encodingAESKey': '', 'token': '', 'callbackPort': 3003, 'allowedUsers': []}
        }
        self.llm_proxy_url = 'http://localhost:3001'
        self.lock = threading.Lock()
        self._wechat_running = threading.Event()
        self._wechat_stop = threading.Event()  # ★ 专门的停止信号（set=停止）
        ...
        try:
            if self._config_file.exists():
                with open(self._config_file, 'r', encoding='utf-8') as f:
                    saved = json.load(f)
                # 合并保存的配置到默认配置（保留默认值兜底）
                for platform, cfg in saved.items():
                    if platform in self.config:
                        for key, value in cfg.items():
                            # logged_in 每次启动重置
                            if key != 'logged_in':
                                self.config[platform][key] = value
                print(f"[Gateway] 已加载持久化配置: {list(saved.keys())}")
        except Exception as e:
            print(f"[Gateway] 加载配置失败: {e}")

    def _save_config(self):
        """保存配置到文件"""
        try:
            # 保存时排除 logged_in（每次启动重置）
            to_save = {}
            for platform, cfg in self.config.items():
                to_save[platform] = {k: v for k, v in cfg.items() if k != 'logged_in'}
            with open(self._config_file, 'w', encoding='utf-8') as f:
                json.dump(to_save, f, ensure_ascii=False, indent=2)
        except Exception as e:
            print(f"[Gateway] 保存配置失败: {e}")
    ...
    def _analyze_file(self, file_path):
        ...
        try:
            ...
            if not resp.ok:
                ...
                print(f'[Gateway] 文件解析失败 HTTP {resp.status_code}: {err_msg}')
            ...
            print(f'[Gateway] 文件解析成功: {filename}, {page_count}页, {len(parsed_text)}字')

            # 保存活跃案例上下文（Bot后续命令自动使用）
            state.active_case_text = parsed_text
            state.active_case_title = filename
            state.active_case_id = f'case_{int(time.time())}'
            # 同步通知后端持久化
            try:
                import requests as _sync
                _sync.post(
                    'http://localhost:3001/api/gateway/active-case',
                    json={'caseId': state.active_case_id, 'title': filename, 'caseText': parsed_text},
            ...
            err_str = str(e).encode('utf-8', errors='replace').decode('utf-8', errors='replace')
            print(f'[Gateway] 文件分析失败: {err_str}')
            return f'文件分析出错: {err_str}'
    ...
    def wechat_message_handler(self, bot, msg):
        ...
        # 检测文件/图片消息
        if not text:
            print(f'[WX] 收到非文本消息 from {uid}: type={msg.get("message_type", "?")}')
            # 使用增强的文件信息提取
            file_info = bot.extract_file_info(msg)
            if file_info:
                file_id, file_name, item_type, dl_method = file_info
                print(f'[WX] 检测到{item_type}(下载方式:{dl_method}): name={file_name}')
                file_path = bot.download_media(msg)
                if file_path:
                    print(f'[WX] 文件已下载: {file_path}')
                ...
                print(f'[WX] 未知消息类型，item_list types={item_types}')
                bot.send_text(uid, f'⚠️ 暂不支持该类型消息。请发送文本或 PDF 文件。', context_token=ctx)
            return

        print(f'[WX] 收到消息 from {uid}: {text[:80]}')
        ...
        if workflow_match:
            ...
            def _run_workflow():
                try:
                    import requests as _req
                    # 创建工作流（注入活跃案例文本）
                    create_payload = {'templateId': workflow_match, 'name': display_name}
                    if state.active_case_text:
                        create_payload['caseText'] = state.active_case_text
                        create_payload['caseTitle'] = state.active_case_title
    ...
    def _push_to_feishu(self, data):
        ...
        bot = state.bots.get('feishu')
        if not bot:
            return {'ok': False, 'message': '飞书未连接，请先启动飞书 Bot'}

        # 如果没有指定接收者，尝试从最近消息中获取
        if not to_user_id and hasattr(bot, '_last_user_open_id') and bot._last_user_open_id:
            to_user_id = bot._last_user_open_id

        if not to_user_id:
            return {'ok': False, 'message': '无接收者。请先从飞书发一条消息给Bot，系统会自动记录对方。'}

        # 如果有标题，先发标题
        if title:
            try:
                bot._send_text(to_user_id, f'📋 {title}')
                time.sleep(0.5)
            except Exception as e:
                print(f'[Feishu] 推送标题失败: {e}')
        ...
        for i in range(0, len(content), max_len):
            part = content[i:i+max_len]
            try:
                bot._send_text(to_user_id, part)
                sent += 1
                time.sleep(0.5)
            except Exception as e:
                print(f'[Feishu] 推送内容失败: {e}')
                record = {
                    'id': str(int(time.time() * 1000)),
                    'time': time.strftime('%Y-%m-%d %H:%M:%S'),
                    'title': title or '(无标题)',
...
class GatewayHandler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if len(path_parts) >= 2 and path_parts[0] == 'config' and path_parts[1] == 'feishu':
            with state.lock:
                state.config['feishu']['enabled'] = data.get('enabled', True)
        ...
        # POST /config/wecom - 配置企业微信
        if len(path_parts) >= 2 and path_parts[0] == 'config' and path_parts[1] == 'wecom':
            with state.lock:
                state.config['wecom']['enabled'] = data.get('enabled', False)
                state.config['wecom']['corpId'] = data.get('corpId', '')
                state.config['wecom']['agentId'] = data.get('agentId', 0)
                state.config['wecom']['secret'] = data.get('secret', '')
                state.config['wecom']['encodingAESKey'] = data.get('encodingAESKey', '')
                state.config['wecom']['token'] = data.get('token', '')
                state.config['wecom']['callbackPort'] = data.get('callbackPort', 3003)
                state.config['wecom']['allowedUsers'] = data.get('allowedUsers', [])
            state._save_config()
            self.send_json({'status': 'ok', 'message': '企业微信配置已更新'})
            return
        ...
        # POST /start/wecom - 启动企业微信
        if len(path_parts) == 2 and path_parts[0] == 'start' and path_parts[1] == 'wecom':
            self._start_wecom()
            self.send_json({'status': 'ok', 'message': '企业微信 Bot 启动中'})
            return
        ...
        # POST /push-feishu - 从前端推送内容到飞书
        if len(path_parts) == 1 and path_parts[0] == 'push-feishu':
            result = state._push_to_feishu(data)
            if result.get('ok'):
                self.send_json({'status': 'ok', 'message': result['message']})
            else:
                self.send_json({'status': 'error', 'error': result['message']}, 400)
        ...
        # POST /push-wecom - 从前端推送内容到企业微信
        if len(path_parts) == 1 and path_parts[0] == 'push-wecom':
            result = state._push_to_wecom(data)
            if result.get('ok'):
                self.send_json({'status': 'ok', 'message': result['message']})
            else:
                self.send_json({'status': 'error', 'error': result['message']}, 400)
    ...
    def _start_wechat(self):
        ...
        # ★ 等待旧的 run_loop 线程完全退出（最多等 5 秒）
        if state._wechat_running.is_set():
            print("[Gateway] 等待旧微信监听线程退出...")
            # _wechat_running 在 run_loop 开始时 set，结束时 clear
            # 所以要等待它变为 clear
            waited = 0
            while state._wechat_running.is_set() and waited < 5:
                time.sleep(0.5)
                waited += 0.5
            time.sleep(0.5)  # 额外缓冲
        ...
        if bot.token and bot.bot_id:
            # ★ 增强 token 验证：不只检查 HTTP 状态码，还检查 getupdates 是否正常工作
            try:
                import requests as _req
                test_resp = _req.post(
                ...
                if test_resp.status_code == 401 or test_resp.status_code == 404:
                    ...
                    # ★ 即使 HTTP 200，也要检查返回的 JSON 是否有错误码
                    try:
                        test_data = test_resp.json()
                        errcode = test_data.get('errcode', 0)
                        if errcode != 0:
                            print(f"[Gateway] 微信 token 返回错误码 errcode={errcode}，删除旧 token，重新扫码")
                            try:
                                from bots.wechat_bot import TOKEN_FILE
                                TOKEN_FILE.unlink(missing_ok=True)
                            except:
                        ...
                        # JSON 解析失败也视为无效
                        print("[Gateway] 微信 token 响应解析失败，重新扫码")
                        try:
                            from bots.wechat_bot import TOKEN_FILE
                            TOKEN_FILE.unlink(missing_ok=True)
                        except:
    ...
    def _run_wechat_loop(self, bot):
        """在新线程中运行微信消息监听循环。"""
        state._wechat_stop.clear()  # 清除停止信号
        def _loop():
            state._wechat_running.set()
            try:
                # ★ 传递 stop_event 让 run_loop 可以被优雅终止
                bot.run_loop(state.wechat_message_handler, stop_event=state._wechat_stop)
            except TypeError:
                # 兼容旧版 run_loop（没有 stop_event 参数），直接调用
                bot.run_loop(state.wechat_message_handler)
            except Exception as e:
                print(f"[Gateway] 微信监听循环异常: {e}")
            finally:
                state._wechat_running.clear()
                state._wechat_stop.clear()
                with state.lock:
                    if state.bots.get('wechat') is bot:
    ...
    def _stop_wechat(self):
        """停止微信 Bot：发送停止信号 + 清空实例 + 删除 token 文件，确保重新登录必须重新扫码"""
        # ★ 发送停止信号让 run_loop 退出
        state._wechat_stop.set()
        # ★ 等待 run_loop 线程退出（最多等 poll_timeout + 5s）
        if state._wechat_running.is_set():
            state._wechat_running.wait(timeout=35)
            time.sleep(0.5)
        ...
        # ── 关键修复：删除 TOKEN_FILE，强制下次启动重新扫码 ────────────────
        try:
            from bots.wechat_bot import TOKEN_FILE
            if TOKEN_FILE.exists():
                TOKEN_FILE.unlink()
                print("[Gateway] 微信 token 文件已删除")
        except Exception as e:
            print(f"[Gateway] 删除 token 文件失败: {e}")
        ...
        def run_feishu():
            try:
                bot = FeishuBot(
                    ...
                    allowed_users=config.get('allowedUsers', []),
                    llm_proxy_url=state.llm_proxy_url,
                    gateway_state=state  # 传入 GatewayState 引用，共享 _analyze_file / session 等
                )
                with state.lock:
    ...
    def _start_wecom(self):
        """启动企业微信 Bot"""
        if not WeComBot:
            print("[Gateway] 企业微信Bot模块未加载")
            return
        if state.bots['wecom'] is not None:
            print("[Gateway] 企业微信 Bot 已在运行")
            return

        config = state.config['wecom']
        if not config.get('corpId') or not config.get('secret'):
            print("[Gateway] 企业微信配置不完整，请先配置 CorpID、AgentID 和 Secret")
            return
        ...
        def run_wecom():
            try:
                bot = WeComBot(
                    ...
                    agent_id=config.get('agentId', 0),
                    secret=config.get('secret', ''),
                    encoding_aes_key=config.get('encodingAESKey', ''),
                    token=config.get('token', ''),
                    allowed_users=config.get('allowedUsers', []),
                    callback_port=config.get('callbackPort', 3003),
                    llm_proxy_url=state.llm_proxy_url,
                    gateway_state=state
                )
                with state.lock:
                    state.bots['wecom'] = bot
                    state.config['wecom']['logged_in'] = True
                bot.run()
            except Exception as e:
                print(f"[Gateway] 企业微信 Bot 错误: {e}")
            finally:
                with state.lock:
                    state.bots['wecom'] = None
                    state.config['wecom']['logged_in'] = False

        thread = threading.Thread(target=run_wecom, daemon=True)
        thread.start()
        state.bot_threads['wecom'] = thread
        print("[Gateway] 企业微信 Bot 启动线程已创建")

    def _stop_wecom(self):
        """停止企业微信 Bot"""
        with state.lock:
            if state.bots['wecom'] is not None:
                try:
                    state.bots['wecom'].stop()
                except:
                    pass
                state.bots['wecom'] = None
                state.config['wecom']['logged_in'] = False
        print("[Gateway] 企业微信 Bot 已停止")

    def _push_to_wecom(self, data):
        """从前端推送内容到企业微信"""
        content = data.get('content', '') if data else ''
        title = data.get('title', '') if data else ''
        to_user_id = data.get('to_user_id', '') if data else ''
        ...
        bot = state.bots.get('wecom')
        if not bot:
            return {'ok': False, 'message': '企业微信未连接，请先启动企业微信 Bot'}

        if not to_user_id and hasattr(bot, '_last_user_id') and bot._last_user_id:
            to_user_id = bot._last_user_id

        if not to_user_id:
            return {'ok': False, 'message': '无接收者。请先从企业微信发一条消息给Bot。'}
...
def run_gateway(port=3002):
    ...
    # 自动启动已启用的 Bot（在后台线程中，避免阻塞 HTTP 服务器启动）
    def auto_start():
        time.sleep(2)  # 等待 HTTP 服务器完全就绪
        # 使用 HTTP 请求触发 _start_all_bots（与前端行为一致）
        try:
            import urllib.request
            req = urllib.request.Request(f'http://127.0.0.1:{port}/start', method='POST',
                                         data=b'', headers={'Content-Length': '0'})
            urllib.request.urlopen(req, timeout=5)
        except Exception as e:
            print(f"[Gateway] 自动启动Bot失败: {e}")
