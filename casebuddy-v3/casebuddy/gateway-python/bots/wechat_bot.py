...
class WxBotClient:
    ...
    def login_qr(self, poll_interval=2, qr_output_path=None):
        ...
        if url:
            # 保存二维码图片（使用更大的 box_size 确保清晰）
            qr = qrcode.QRCode(border=2, box_size=10)
            qr.add_data(url)
            qr.make(fit=True)
            img = qr.make_image(fill_color='black', back_color='white')
            img.save(qr_output_path, quality=95)
            print(f'[WX-QR] 二维码已保存: {qr_output_path}', file=sys.__stdout__)
            # 尝试用系统默认程序打开二维码图片（优先用 os.startfile 在 Windows 上更可靠）
            try:
                abs_path = os.path.abspath(qr_output_path)
    ...
    def run_loop(self, on_message, poll_timeout=30, stop_event=None):
        """主循环：监听用户消息并回调 on_message(bot, msg)。
        stop_event: threading.Event，设置后循环退出。
        """
        print(f'[WX] 监听中... (bot_id={self.bot_id})', file=sys.__stdout__)
        seen = set()
        while True:
            if stop_event and stop_event.is_set():
                print('[WX] 收到停止信号，退出监听', file=sys.__stdout__)
                break
            try:
