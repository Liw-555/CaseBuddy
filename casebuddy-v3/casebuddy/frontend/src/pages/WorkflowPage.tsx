      ...
      pollRef.current = setInterval(async () => {
        try {
          ...
          if (resp.ok) {
            ...
            // 自动展开正在运行的步骤，让用户看到实时输出
            const runningStep = data.workflow.steps.find((s: WorkflowStep) => s.status === 'running');
            if (runningStep) {
  ...
  return (
    <div className="h-full flex flex-col bg-surface-50">
      ...
      <div className="flex-1 overflow-y-auto scrollbar-thin">
        {activeTab === 'history' ? (
          <HistoryView history={history} onViewWorkflow={(wf) => { setCurrentWorkflow(wf); setActiveTab('workflow'); }} onPush={handlePushToWechat} pushing={pushing} />
        ) : (
          <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            {currentWorkflow && (() => {
              ...
              const failedSteps = currentWorkflow.steps.filter(s => s.status === 'failed').length;
              const progressPct = totalSteps > 0 ? Math.round(((completedSteps + failedSteps) / totalSteps) * 100) : 0;
              const runningStep = currentWorkflow.steps.find(s => s.status === 'running');
              ...
              <section className="animate-fade-in-scale">
                ...
                <div className="flex items-center justify-between mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-medium ${
                      currentWorkflow.status === 'completed' ? 'text-emerald-600' :
                      currentWorkflow.status === 'failed' ? 'text-red-600' : 'text-blue-600'
                    }`}>
                      {statusConfig[currentWorkflow.status]?.label || currentWorkflow.status}
                      {currentWorkflow.status === 'running' && ` (${completedSteps + failedSteps}/${totalSteps})`}
                    </span>
                  </div>
                </div>
                ...
                {currentWorkflow.status === 'running' && (
                  <div className="mb-4 bg-white rounded-lg border border-surface-200 p-3">
                    <div className="flex items-center justify-between text-xs text-surface-500 mb-2">
                      <span>分析进度</span>
                      <span>{completedSteps} 完成 / {totalSteps} 总计</span>
                    </div>
                ...
                <div className="bg-white rounded-xl border border-surface-200 overflow-hidden">
                  {currentWorkflow.steps.map((step, i) => {
                    ...
                    return (
                      <div key={step.id} className={`border-b border-surface-100 last:border-0 ${
                        step.status === 'running' ? 'bg-blue-50/30 ring-1 ring-blue-200 ring-inset' : ''
                      }`}>
                        {/* 步骤头部 */}
                        <button
                          onClick={() => (hasResult || step.result?.preview) ? toggleStep(step.id) : undefined}
                          className="w-full px-5 py-3.5 flex items-center gap-3 text-left hover:bg-surface-50/50 transition-colors"
                        >
                          {/* 步骤序号 */}
                          <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${
                            step.status === 'completed' ? 'bg-emerald-100 text-emerald-700' :
                            step.status === 'failed' ? 'bg-red-100 text-red-700' :
                            step.status === 'running' ? 'bg-blue-500 text-white shadow-sm shadow-blue-200' :
                          ...
                          {step.result?.length && (
                            <span className="text-xs text-surface-400 bg-surface-100 px-1.5 py-0.5 rounded">
                              {step.result.length.toLocaleString()} 字
                            </span>
                          )}

                          {(hasResult || step.result?.preview) && (
                            isExpanded ? <ChevronUp className="w-4 h-4 text-surface-400" /> : <ChevronDown className="w-4 h-4 text-surface-400" />
                          )}
                        </button>

                        {/* 展开的详细结果 */}
                        {isExpanded && (hasResult || step.result?.preview) && (
                          <div className="px-5 pb-4 pt-0">
                            {step.status === 'running' && !hasResult && (
                              <div className="flex items-center gap-2 text-xs text-blue-500 mb-2">
                                <Loader2 className="w-3 h-3 animate-spin" />
                                AI 正在生成中，请稍候...
                              </div>
                            )}
                            <div className="bg-surface-50 rounded-lg p-4 text-sm text-surface-700 whitespace-pre-wrap max-h-[500px] overflow-y-auto scrollbar-thin border border-surface-200 leading-relaxed">
                              {currentWorkflow.results[step.name] || step.result?.preview || '（等待输出...）'}
                            </div>
                          </div>
                        )}
